...
* bool puedeConsumir
******
* La función consulta el rut de un colaborador en el archivo binario para luego ver si tiene el dinero para comprar el servicio, si tiene dinero lo registra en un archivo ".txt" y retorna true, si no tiene dinero no se le registra en el archivo y retorna falso.
******
* Input:
* char* rut: rut es un puntero que apunta a una cadena de caracteres.
* int servicio: servicio es un entero, que dependiendo de su valor es lo que significa, si vale 0 se refiere al desayuno, si vale 1 se refiere al almuerzo, si vale 2 se refiere a la cena y si vale 3 se refiere a la cena.
* string consumos_dias: consumos_dias es un string, representa una cadena de caracteres que en este caso es el nombre del archivo sobre el cual queremos escribir.
******
* Returns:
* bool, Esta función retorna true or false, retorna true si es que el colaborador tiene salgo para recibir ticket de alimentación, de lo contrario retorna falso.
*****/
